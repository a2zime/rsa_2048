...
### 2.1 モンゴメリ乗算: FIOS（Finely Integrated Operand Scanning）
...
### 2.2 モジュラー累乗: Left-to-right binary（Square-and-Multiply）
...
- `N` はパラメータのワード数（2048bit=64、1024bit=32、32bit=1）であり、
  `addr_i` から自動決定される。
- `addr_i` は **パラメータ ID**（4bit）であり、ワードアドレスではない。
  1 パラメータ転送中はホスト側で同じ値を保持し、内部のワードカウンタが
  インクリメントされる（詳細は §5.2 io_controller 参照）。
...
### 5.2 io_controller — シリアル I/O コントローラ
...
`addr_i` は **ワードアドレスではなくパラメータ ID**（4bit, `param_addr_e`）である。
1 つのパラメータ転送中、ホスト側は `addr_i` を**同じ値に保持**し、
io_controller が内部でワードカウンタ（`word_cnt`）を Valid/Ready ハンドシェイク成立ごとに
0 → N-1 までインクリメントする。実際のメモリアドレスは次のように生成する：

```
mem_addr_o = param_base_addr(addr_i) + word_cnt
```

ここで `param_base_addr()` は `rsa_pkg::param_addr_e` から物理ベースアドレスへの
デコード関数（定数テーブル）である。

**なぜ「パラメータ ID + 内部カウンタ」方式を採用したか:**

代替案として「外部から毎ワードごとにフルメモリアドレスを与える」方式も考えられるが、
以下の理由で不採用とした。
...
| 転送境界の検出 | `word_cnt = N-1` で load_done をパルス生成 → FSM が次状態へ遷移可能 | 境界情報がバス上に無い／別途 last 信号が必要 |
| 誤アドレスリスク | パラメータ領域外への書き込み不可（デコーダが安全側） | ホストがアドレスを間違えると任意領域を上書きし得る |
| 転送順序 | LSB-first を IP 側で強制 | ホスト側の責務、間違いやすい |
| ピン数 | 少ない（組込み用途向き） | 多い |

**「外部からアドレスを毎回インクリメントし、内部では自動カウントしない」方式の問題点:**

1. **転送境界情報の喪失** — パラメータ転送の最終ワードを IP が知る手段が無くなるため、
   `load_done` を生成するには別途 `last_i` 信号が必要となる。
2. **パラメータ単位のアトミック性が崩れる** — ホスト側のアドレス計算ミスで
   複数パラメータ領域にまたがる書き込みが発生し得る。
3. **ホスト側ドライバが複雑化** — C ドライバ / ソフト側で毎ワードごとに
   アドレスを計算する必要があり、DMA 転送との親和性も低い。
...
### 5.5 mul_add_unit — 32×32 積和演算ユニット
...
### 7.1 FPGA リソース（Spartan-7 XC7S25）
