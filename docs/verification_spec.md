...
| ツール | 用途 | バージョン |
|---|---|---|
| Icarus Verilog（`iverilog`） | テストベンチ実行（機能検証主体） | 12.0+ |
...
| MM-08 | 最終条件付き減算が発火するケース | 境界値 | 中間 t ≥ n となる入力 | `t - n` が得られること | Python参照と DUT result_o を bit-exact比較。波形で内部 `sub_en` 信号がアサートされることを確認 |
| MM-09 | 最終条件付き減算が発火しないケース | 境界値 | 中間 t < n のままとなる入力 | `t` がそのまま得られること | Python参照と DUT result_o を bit-exact比較。波形で内部 `sub_en` 信号が非アサートであることを確認 |
...
| MM-12 | 内部ループサイクル数 | タイミング | 2048bit MontMul の done 時刻 | 25K サイクル以内 | サイクルカウント（start_i〜done_o 間）を計測し、25,000 以下であることを確認 |
